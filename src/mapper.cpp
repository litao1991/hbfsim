...
PhysicalAddr AddressMapper::base_map_channel(
  ...
  remaining /= config_.blocks_per_plane;
  const auto flat_plane = static_cast<std::uint64_t>(address.channel) *
                          planes_per_channel + remaining;
  result.stack = static_cast<std::uint32_t>(flat_plane / planes_per_stack);
  const auto local_plane = flat_plane % planes_per_stack;
  result.die =
      static_cast<std::uint32_t>(local_plane / config_.planes_per_die);
  result.plane =
      static_cast<std::uint32_t>(local_plane % config_.planes_per_die);
  result.data_port =
