...
std::uint64_t Simulator::start_gc_erase_only(
  ...
  CopyJob job;
  job.id = system_.copy_engine().next_job_id();
  ...
  system_.copy_engine().jobs().emplace(id, std::move(job));
...
void Simulator::enqueue_copy_read(std::uint64_t job_id, std::uint32_t slot,
                                  SimTime now) {
  auto& job = system_.copy_engine().jobs().at(job_id);
  auto* mapping = system_.mapper().stripe_mapping();
...
void Simulator::enqueue_stripe_erases(
  ...
  for (const auto& address : blocks) {
    ...
    request.host_route = system_.host_router().route(0, address);
    const auto command = reserve_host(request.host_route,
                                      HostLinkDirection::Command, now, 64,
                                      measured);
    request.host_command_wait_ns = command.start - now;
...
void Simulator::handle_copy_completion(
  ...
  const auto job_it = system_.copy_engine().jobs().find(job_id);
  if (job_it == system_.copy_engine().jobs().end()) return;
...
void Simulator::finish_copy_job(std::uint64_t job_id, SimTime now,
                                bool failed) {
  const auto it = system_.copy_engine().jobs().find(job_id);
  if (it == system_.copy_engine().jobs().end()) return;
  ...
  system_.copy_engine().jobs().erase(it);
}
